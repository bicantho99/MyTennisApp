...
import { useAttackStore } from "../../../assets/useAttackStore";

export default function Adding() {
  const { addAttack, loadAttack, deleteAttack, challenges } = useAttackStore();
  const [text, setName] = useState("");
  const [descr, setDescr] = useState("");
  const randomId = uuid.v4();
  const handleSubmit = () => {
    const newChallenge = { text, descr, randomId };
    addAttack(newChallenge);
  };
  return (
            ...
            renderItem={({ item }) => (
              <View className="rounded-xl gap-3 p-2  mt-[8.3px]">
                <View className="flex-row justify-between items-center">
                  ...
                  <TouchableOpacity onPress={() => deleteAttack(item.id)}>
